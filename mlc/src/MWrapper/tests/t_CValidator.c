...
/***************************************************************************
  Description  : Tests the methods of the CrossValidator class
		    ...
		    C4.5 on t_CValidator-0-0, and -0-1 and averaging.
  Doesn't test :
  Enhancements :
...
RCSID("MLC++, $RCSfile: t_CValidator.c,v $ $Revision: 1.36 $")

const MString fileStem  = "/tmp/t_lensesCV";
const MString namesFile = "t_CValidator.names";
const MString dataFile  = "t_CValidator.data";
...
void cleanup()
{
   ...
   MLCOStream dummy1("t_CValidator-0-X.names");
   dummy1.close();

   system("csh -c \"rm t_CValidator-0-*.{names,data,test,tree,unpruned}\""
	  "< /dev/null");
}
...
main()
{
   ...
   ID3Inducer id3Inducer("t_CValidator id3 inducer");
   ...
   CrossValidator cv(NUM_FOLDS, NUM_TIMES);
   cv.set_log_level(3);
   ...
   if (cv.get_acc_data() != cv2.get_acc_data()) 
      err << "Array CV " << (StatData&)cv.get_acc_data() 
	  << " not equal to array CV2 " << (StatData&)cv2.get_acc_data()
	  << fatal_error;
   ...
   C45CV.dump_files(bag, "t_CValidator");

   // Run C4.5 "manually," i.e., without using CValidator
   InstanceList train0("t_CValidator-0-0");
   InstanceList test0 ("", "t_CValidator-0-0.names", "t_CValidator-0-0.test");
   InstanceList train1("t_CValidator-0-1");
   InstanceList test1 ("", "t_CValidator-0-1.names", "t_CValidator-0-1.test");
   ...
   // Try the auto-fold method.
   C45CV.init_rand_num_gen(RAND_SEED);
   C45CV.set_log_level(2);
   C45CV.auto_set_folds(c45Inducer, bag, 10, 5);
   C45CV.auto_estimate_accuracy(c45Inducer, bag);
