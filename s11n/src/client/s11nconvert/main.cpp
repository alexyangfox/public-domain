...
{
	os << s11n_S11N_LIBRARY_VERSION << "\n";
	if( s11nc_verbose) {
		os << "Built at " << __TIME__ << " on " << __DATE__ << ".\n"
		   << "Linked against s11n version " << s11n::library_version() << ".\n"
...
#define HOP(A,H) os << "\t" << A << ":\n\t\t" << H << "\n";
	...
	HOP("-a, --autoload", "Try to automatically load certain addons. Use -v to see what gets autoloaded." );
#else
	HOP("-dl PLUGIN, -DL PLUGIN", "plugins support is disabled in your build of libs11n, so the -dl and -DL options are not available.");
#endif
...
#else
	HOP("-z, -bz","libzfstream support is disabled in your build of libs11n, so the -z and -bz options are not available.");
#endif // zfstream?
...
#define APPERR std::cerr << "s11nconvert error: "
	...
	// Autoload "known" plugins:
	if( NTR::is_set( argn, "a" ) || NTR::is_set( argn, "autoload" ) )
	{
#if ! s11n_CONFIG_ENABLE_PLUGINS
		throw std::runtime_error( "Your s11n was not built with plugins support, so the -a option cannot be used." );
#else
		typedef std::vector<std::string> SVec;
		SVec addons;
#if s11n_CONFIG_ADDON_PSTREAMS_ENABLE
		addons.push_back( "pstreams" );
#endif // s11n_CONFIG_ADDON_PSTREAMS_ENABLE
		SVec::const_iterator it, et;
		it = addons.begin();
		et = addons.end();
		std::string found;
		for( ; et != it; ++it )
		{
			found = s11n::plugin::open( *it );
			VERBOSE << "Autoloading plugin ["<<(*it)<<"]: "
				<< (found.empty() ? std::string("not loaded") : found)
				<< std::endl;
		}
#endif // s11n_CONFIG_ENABLE_PLUGINS
	}

	load_dlls( argc, argv ); // do this after verbosity check and autoloads
        ...
        {
                APPERR << "-z and -bz may not be used together." << std::endl;
        ...
        if( fmt.empty() )
        {
                APPERR << "No output format (serializer) specified. "
         ...
         APSER ser( s11nlite::create_serializer( fmt ) );
         if( ! ser.get() )
         {
                 APPERR << "No Serializer found for name '"<<fmt<<"'. Try one of:\n"
                     << known_ser << "\n";
                 return ErrorNoOutputSerializer;
         }
...
#else
		 APPERR << "zlib/bzip2 compression is not supported by your libs11n.\n";
		 return ErrorUsageError;
#endif
	 }
	 ...
	 std::string ifname = NTR::get( argn, "f", NTR::get( argn, "file", std::string() ) );
         ...
         {
                 APPERR << "No input file specified.\n";
	 ...
	 std::string ofname = NTR::get( argn, "o", NTR::get(argn, "output", std::string() ) );
         ...
         if( ofname.empty() || ("-" == ofname) )
         {
                 if( usegz || usebz )
                 {
                         APPERR << "Compression (-z and -bz) works only for files, not cout." << std::endl;
                         return ErrorUsageError;
                 }
                 os = &std::cout;
         }
         ...
         if( ! forcedinser.empty() )
         {
                 ...
                 if( 0 == forceser )
                 {
                         APPERR << "Could not load forced input serializer '"<<forcedinser<<"'!\n";
         ...
         if( ! innode.get() )
         {
                 APPERR << "Error loading node tree from input stream!\n";
