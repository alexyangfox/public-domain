...
namespace s11n {
		...
		/**
		   ...
		   As of version 1.3.0, this function supports URLs.
		   URLs will be dispatched to get_url_ostream().

		   When passed a non-URL filename, the returned stream
		   supports libzl and libbz2 if your libs11n is built
		   with libs11n_zfstream support, meaning it can read
		   files compressed with zlib/gzip or bz2lib.
		*/
		...
		/**
		   ...
		   When passed a non-URL filename the returned stream
		   supports libzl and libbz2 if your libs11n is built
		   with libs11n_zfstream support. That means that if
		   zfstream::compression_policy() is set, then the
		   returned string might be a compressing stream.

		   In version 1.3.0 the 2nd parameter (ExternalData)
		   was removed because it is never used.
		*/
		std::istream * get_istream( const std::string name );
		   ...
		   See get_magic_cookie( istream & ) for notes on a
		   minor functional change introduced in version
		   1.2.1.

		   Changed in 1.3.0: second parameter to this function
		   (bool ExternalData) was removed.
                */
                        ...
                        virtual ~data_node_serializer()
			{
			   ...
			   this function no longer leaks.
				...
				if( destfile.empty() ) return false;
                                s11n::Detail::auto_ptr<std::ostream> os( ::s11n::io::get_ostream( destfile ) );
			   ...
			   whole stream content if they need to.
                        ...
                        */
                        ...
                        {
                                typedef s11n::Detail::auto_ptr<std::istream> AP;
                                AP is( ::s11n::io::get_istream( src ) );
                                if( ! is.get() ) return 0;
		...
		*/
		...
		{
			s11n::Detail::auto_ptr<std::istream> is( get_istream( infile.c_str() ) );
			return is.get()
		...
		stream.
                ...
                */
                ...
                {
			typedef data_node_serializer<NodeType> ST;
			s11n::Detail::auto_ptr<ST> ser( guess_serializer<NodeType>( is ) );
			return ser.get()
				? ser->deserialize( is )
				: 0;
                }
                ...
                /**
                   Similar to load_node( istream ), overloaded to take
                   a filename. This is handled separately from the
                   stream overload because some Serializers must
                   behave differently in the face of streams. e.g.,
                   db-based Serializers typically can't deal with
                   streams.
		   ...
		   Behaviour change in 1.3.0: The second argument to
		   this function (bool ExternalData) was removed.
                */
                template <typename NodeType>
                NodeType * load_node( const std::string & src )
                {
			typedef data_node_serializer<NodeType> ST;
			s11n::Detail::auto_ptr<ST> ser( guess_serializer<NodeType>( src ) );
			return ser.get()
				? ser->deserialize( src )
				: 0;
                }
                ...
                */
                ...
                {
                        typedef s11n::Detail::auto_ptr<NodeT> AP;
		   ...
		   Changed in 1.3.0: the second parameter to
		   this function (bool ExternalData) was removed.
                */
                template <typename NodeT,typename SerializableT>
