...
//                         > TMap;
	...
	void default_escapes_initializer::operator()( entity_map & map )
	{
        ...
        void string_tokenizer::tokenize( const char *str, const char *_separators )
        {
                if ( *str == '\0' )
                {
                        pos = 0;
        ...
        const char* string_tokenizer::next_token()
        {
                if ( pos == 0 )
                        return 0;
        ...
        bool string_tokenizer::has_tokens() { return ( pos != 0 ); }
        ...
        stdstring_tokenizer::stdstring_tokenizer()
	{
        ...
        stdstring_tokenizer::stdstring_tokenizer( const std::string & str, const std::string & separators )
        {
                this->tokenize( str, separators );
        }

	void stdstring_tokenizer::tokenize( const std::string & str, const std::string & sep )
	{
		const std::string::size_type ssize = str.size();
		if ( ssize == 0 )
			return;
		if ( ssize == 1 )
		{
		...
		if ( std::string::npos == str.find_first_of( sep ) )
		{
			this->m_list.push( str );	// += str;
			return;
		}
		...
		std::string mystr;
		bool addit;
		for ( std::string::size_type pos = 0; ( pos < ssize ); pos++ )
		{
			chat = str[pos];
			addit = true;
			if ( ( chat == '\"' ) && !quoted )
	...
	std::string stdstring_tokenizer::next_token()
	{
		...
		std::string foo = this->m_list.front();
		this->m_list.pop();
		return foo;
	}
