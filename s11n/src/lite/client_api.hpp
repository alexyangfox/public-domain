...
namespace s11nlite {
	...
	/**
	   ...
	   While this type's interface does not require it, it is
	   believed that all of the member functions of client_api's
	   base implementation are thread-safe and reentrant EXCEPT
	   for the serializer_class() getter/setter pair. Even with
	   thread locking, there are a lot of possible race conditions
	   regarding setting that value and fetching it. Since the
	   setter is rarely used (typically once per application),
	...
	*/
	...
	public:
		...
		client_api()
			: m_serclass(s11n_S11NLITE_DEFAULT_SERIALIZER_TYPE_NAME /* defined in s11n_config.hpp */ ),
			  m_mutex()
		{
		}

		explicit client_api( const std::string & default_serializer_class )
			: m_serclass(default_serializer_class),
			  m_mutex()
		{
		}
		...
		*/
		inline serializer_interface * create_serializer()
		{
			...
			return this->create_serializer( this->m_serclass );
		}
		...
		*/
		...
		{
			s11n::mutex_sentry lock(this->m_mutex);
			// ^^^ keep m_serclass from changing during create_serializer()
		...
		*/
		virtual bool save( const node_type & src, const std::string & filename )
		{
			...
			// ps11n support also distinguishes, as it supports
