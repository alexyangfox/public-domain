...
DeserializableT * s11n::deserialize( const DataNodeType & src )
{
	...
	typedef typename s11n::type_traits<DeserializableT>::type base_type;
	typedef s11n::cleanup_ptr<base_type> CLEAN;
	FAC fac;
	CLEAN obj( fac( NTR::class_name( src ) ) );
	if( ! obj.get() )
	{
		using namespace ::s11n::debug;
		S11N_TRACE(TRACE_ERROR)
			...
			<< NTR::class_name( src )<<"'."
			<< " It is probably not registered with its base-most ("
			<< STR::class_name(0)<<") classloader.";
		return 0;
	}
	try
	{
		if( s11n::deserialize<DataNodeType,base_type>( src, *obj ) )
		{
	...
	catch( ... )
	{
		using namespace ::s11n::debug;
		S11N_TRACE(TRACE_ERROR)
		    << "deserialize<NT,"<<STR::class_name(0)<<">(const NT&"
		    << "[class="<<NTR::class_name(src)<<"]) failed deser'ing child and is using cleanup_serializable() on it.\n";
		throw;
	}
	return 0;
}
...
template <typename NodeType, typename FromType, typename ToType>
bool s11n::s11n_cast( const FromType & t1, ToType & t2 )
{
	NodeType n;
	return ::s11n::serialize<NodeType,FromType>( n, t1 ) && ::s11n::deserialize<NodeType,ToType>( n, t2 );
}
