...
variant user = ::getenv(arg);
    ...
    */
    ...
    /**
       Reads from the input stream, appending to v.str() until the stream
    ...
    */
    inline std::istream & operator>>( std::istream & is, variant & v )
    {
        /****
        ...
        ****/
        /*****
              while( std::getline( is, v.str() ).good() );
              // ^^^ eeek! strips newlines!
              *****/
        /*****
              is >> v.str(); // depends on skipws.
        *****/
        /*****
        ...
        *****/
        ...
        std::getline( is, v.str(), static_cast<std::istream::char_type>(std::istream::traits_type::eof()) );
        return is;
    }
