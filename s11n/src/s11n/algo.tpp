#ifndef s11n_net_s11n_v1_3_ALGO_TPP_INCLUDED
#define s11n_net_s11n_v1_3_ALGO_TPP_INCLUDED
			      ...
			      const SerializableT & src )
{
	...
	Detail::auto_ptr<DataNodeType> sub(NT::create(nodename));
...
bool s11n::deserialize_versioned( NodeType const & src, VersionType const ver, SerType & dest )
{
    ...
    if( ! ch )
    {
	throw s11n_exception( S11N_SOURCEINFO, "Data version matched but versioned data not found." );
    }
    return deserialize( *ch, dest );
}


#endif // s11n_net_s11n_v1_3_ALGO_TPP_INCLUDED
