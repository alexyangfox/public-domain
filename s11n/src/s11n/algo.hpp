#ifndef s11n_net_s11n_v1_3_ALGO_HPP_INCLUDED
#define s11n_net_s11n_v1_3_ALGO_HPP_INCLUDED 1
/////////////////////////////////////////////////////////////////////////
...
#include <s11n.net/s11n/exception.hpp> // source_info class
...
namespace s11n
{
	   ...
	   DO NOT use this functor for Serializables - use
	   cleanup_serializable() instead.
        */
...
//                 const_object_reference_wrapper() : m_ptr(0) {}
	...
	namespace Detail {
	   ...
	   incorrect data. (Huh? Is that true?)
        */
	...
	namespace debug {
		...
		/**
		   ...
		   in recursion: do not pass a value for it, or pass 0 if you
		   absolutely must pass something.
		*/
		template <typename NodeT>
	...
	*/
	...
	/**
	   This function behaves similar to snprintf(), but returns
	   the result as a std::string.

	   buffsize is the maximum size of expanded format strings for
	   vsnprintf(). This argument "should" be a size_t, but an int
	   greatly simplifies the integration with vsnprintf (requires
	   no casting and can cause no under/overflows). If buffsize
	   is 0 or negative then a "reasonable" buffer size is used,
	   where "reasonable" is defined as some unspecified value, possibly
	   1k, possibly 4k, possibly neither.
	   ...
	   the text continues. Thus this function is more suited to
	   general error/debug reporting and not data serialization.

	   If your function is a variadic function, you can pass them on
	   to this function by doing the following...
	   
	   Assuming your function looks like:

	   int my_variadic( char const * format, ... );

	   Then, from my_variadic() we could pass on the parameters
	   to format_string() like so:
	...
	*/
	...
	/**
	   ...
	   format_string(buffsize,format,va_list).
	*/
	std::string format_string( int buffsize,
				   const char *format,
				   ... ) throw();
	...
	/**
	   ...
	   file/line/function information to the string. The info
	   from si is not counted against the size limit defined by
	   buffsize, so the returned string may legally be larger than
	   buffsize.
	*/
	...
	/**
	   Functionally identical to
	   format_string(si,buffsize,format,va_list).
	*/
	std::string format_string(  source_info const si, 
				    int buffsize,
				    const char *format,
	...
	/**
	   Functionally identical to
	   format_string(buffsize,format,va_list), but it uses a
	...
	*/
	...
	/**
	   Functionally identical to
	   format_string(source_info,buffsize,format,va_list), but it uses a
	   dynamically-growing buffer.
	*/
	std::string format_string( source_info const si, 
				   const char *format,
...
#endif // s11n_net_s11n_v1_3_ALGO_HPP_INCLUDED
