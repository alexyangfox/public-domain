...
#include <s11n.net/s11n/serialize.hpp> // cleanup_ptr<>
...
//  	}
	...
	*/
	...
	/**
	   ...
	   Added in 1.1.3.
	*/
	struct serialize_binary_f : serialize_binary_f_tag
	{
	...
	/**
	   ...
	   Added in 1.1.3.
	*/
	struct deserialize_binary_f : deserialize_binary_f_tag
	{
		template <typename NT, typename ST>
	...
	*/
	...
	struct serialize_nullary_f : serialize_nullary_f_tag
	{
		...
		/**
		   Functionally identical to the 3-arg ctor with a default-constructed
		   BinaryFunctorT as the third argument.
		*/
		...
		/**
		   ...
		   n and s must both outlive this object. Even though s is a
		   const ref, we do not copy it. We expect s to stay valid.
		*/
	...
	*/
 	template <typename NodeType, typename BinaryFunctorT = serialize_binary_f>
	...
	*/
 	template <typename NodeType, typename BinaryFunctorT = deserialize_binary_f>
	...
	*/
	...
	/**
	   ...
	   Added in 1.1.3.
	*/
 	template <typename SerT, typename BinaryFunctorT = serialize_binary_f>
	...
	*/
	...
	/**
	   ...
	   Added in 1.1.3.
	*/
 	template <typename SerT, typename BinaryFunctorT = deserialize_binary_f>
	...
	*/
	...
	/**
	   ...
	   Added in 1.1.3.
	*/
	template <typename NodeType,typename DeserializableT, typename BinaryFunctorT = deserialize_binary_f>
	...
	*/
	 ...
	 */
	template <typename SerializableT, typename BinaryFunctorT = serialize_binary_f>
	...
	*/
	template <typename DeserializableT, typename BinaryFunctorT = deserialize_binary_f>
	...
	*/
	...
	/**
	   ...
	   Also, we create and copy SerializableTypes here, so that
	   type should be cheap to do that with. Note that only
	   monomorphic Serializables are properly handled by this
	   type!

	   BinaryFunctorT must conform to the interface defined by
	   deserialize_binary_f.
	   ...
	   \endcode
	*/
	template <typename SerializableType, typename OutIterator, typename BinaryFunctorT = deserialize_binary_f>
	struct deserialize_to_output_iter_f
	{
		...
		*/
		...
		/**
		   ...
		   serializable_type (non-polymorphically!) and deserializes it.  On success
		   iterator is assigned and incremented and true is
		   returned.  On error false is returned or an
		...
		*/
		...
		bool operator()( NodeType * const & src )
		{
			...
			{
#if 0
				// TODO: figure out why this classload is failing (test case: serializable_type==int)
				std::string c = node_traits<NodeType>::class_name(*src);
				cleanup_ptr<serializable_type> dest( s11n::cl::classload<serializable_type>( c ) );
				// ^^^ we use classload() here so that we get polymorphism
				// CERR << "class name="<<c<<", dest == @"<<std::hex<<dest.get()<<'\n';
 				if( ! dest.get() )
 				{
 					return false;
 				}
				...
				{
					*(iterator++) = *dest; // achtung: not really polymorphism-friendly
					return true;
				}
#else
	...
	*/
 	template <typename BinaryFunctorT = serialize_binary_f>
 	struct serialize_to_subnode_f : serialize_binary_f_tag
 	{
		...
		*/
 		...
 		inline bool operator()( NodeType & dest, SerializableT const & src ) const
 		{
			...
			Detail::auto_ptr<NodeType> nap( NTR::create( this->name ) );
	...
	*/
 	...
 	/**
	   ...
	   Added in 1.1.3.
	*/
 	template <typename BinaryFunctorT = deserialize_binary_f>
