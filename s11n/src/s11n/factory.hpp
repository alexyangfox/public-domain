	...
	*/
	namespace fac {
		...
		*/
		...
		protected:
			...
			/**
			   ...
			   all instances of factory_mgr.  The reasoning behind
			...
			*/
			...
			{
				return ::s11n::Detail::phoenix<factory_map_type, factory_mgr>::instance();
			}
			...
			/**
			   ...
			   Access to this object is not locked,
			   so clients using multiple threads and
			   factories should not call this if they're
			   worried about the alias list changing while
			   they're using it.
			*/
			aliaser_type & aliases()
			{
				return ::s11n::Detail::phoenix<aliaser_type,factory_mgr>::instance();
			}
		...
		public:
			...
			*/
			...
			/**
			   ...
			   registered factory, such lookups will not
			   be done (this code doesn't care if the
			   factory returns 0 or not).
			...
			*/
			...
			{
 				// return ::s11n::fac::instance_hook<factory_mgr>::instance();
				...
				return ::s11n::Detail::phoenix<factory_mgr>::instance();
			}

			/**
			   Const overload. See the non-const variant for
			   information about threading.

			   The ONLY reason that this function is in
			   the public API is so that
			   s11n::io::serializer_list() can fetch the
			   list of alias names. Don't rely on this
			...
			*/
			...
			/**
			   Aliases Alias to be the same as Real, such that create(Alias)
			   and create(Real) are functionally identical.
			...
			*/
			void alias( std::string const & Alias, std::string const & Real )
			{
				this->aliases().alias( Alias, Real );
			}
		...
		*/
		...
		inline void register_factory( const std::string & classname )
		{
 		...
 		/**
 		   ...
 		   that its create() function simply returns 0 (or
		...
		*/
		...
		/**
		   Same as factory<InterfaceT>().alias(Alias,Real).
		*/
		template <typename InterfaceT>
		inline void alias( std::string const & Alias, const std::string & Real )
		{
			return factory<InterfaceT>().alias( Alias, Real );
		}
