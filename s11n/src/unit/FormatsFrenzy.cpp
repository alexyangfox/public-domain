...
public:
	...
	virtual bool run()
	{
		...
		// ^^^ JS serializer doesn't support reading, which will cause this test to fail
		this->formats.push_back("funtxt");
		this->formats.push_back("compact");
		this->formats.push_back("parens");
		this->formats.push_back("funxml");
		this->formats.push_back("wesnoth");
#if s11n_CONFIG_HAVE_LIBEXPAT
		this->formats.push_back("expat");
#endif
		...
		for( ; e1 != b1; ++b1 )
		{
			...
			s11n::Detail::auto_ptr<s11nlite::serializer_interface> ser1( s11n::cl::classload<s11nlite::serializer_interface>( (*b1) ) );
			UT_ASSERT( ser1.get() );
			...
			for( ; e2 != b2; ++b2 )
			{
				...
				s11n::Detail::auto_ptr<s11nlite::node_type> lnode( s11nlite::load_node( f1 ) );
				UT_ASSERT( lnode.get() );
				std::string f2( this->className()+"."+(*b2)+".s11n" );
				s11n::Detail::auto_ptr<s11nlite::serializer_interface> ser2( s11n::cl::classload<s11nlite::serializer_interface>( (*b2) ) );
				UT_ASSERT( ser2.get() );
