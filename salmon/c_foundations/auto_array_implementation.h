/* file "auto_array_implementation.h" */

/*
 *  This file contains the implementation of a module to implement automatic
 *  allocation of space in an array as it's needed.
 *
...
#ifndef AUTO_ARRAY_IMPLEMENTATION_H
#define AUTO_ARRAY_IMPLEMENTATION_H
 ...
 *  a given string_index with a particular key.  The remove_from_string_index()
 ...
 *  If enter_into_string_index() fails, it will silently exit with no change to
 *  the string_index.  To detect that this has happened, call
 *  exists_in_string_index() right after enter_into_string_index(), using the
 *  same key -- it will return FALSE if and only if the
 *  enter_into_string_index() failed, assuming that the key wasn't already in
 *  the string index.
 *
...
#define AUTO_ARRAY_IMPLEMENTATION(name, type, padding) \
 \
    extern verdict name ## _init(name *auto_array, size_t starting_space) \
      { \
        type *array; \
 \
        assert(auto_array != NULL); \
        assert(starting_space > 0); \
 \
        auto_array->element_count = 0; \
        auto_array->space = starting_space; \
        array = MALLOC_ARRAY(type, starting_space); \
        if (array == NULL) \
            return MISSION_FAILED; \
        auto_array->array = array; \
        return MISSION_ACCOMPLISHED; \
      } \
 \
    extern verdict name ## _append(name *auto_array, type new_element) \
      { \
        verdict the_verdict; \
 \
        assert(auto_array != NULL); \
        assert(auto_array->array != NULL); \
 \
        the_verdict = name ## _make_space(auto_array, \
                                          auto_array->element_count + 1); \
        if (the_verdict != MISSION_ACCOMPLISHED) \
            return the_verdict; \
 \
        auto_array->array[auto_array->element_count] = new_element; \
        ++(auto_array->element_count); \
 \
        return MISSION_ACCOMPLISHED; \
      } \
 \
    extern verdict name ## _append_array(name *auto_array, size_t count, \
            const name ## _element_type *new_elements) \
      { \
        verdict the_verdict; \
 \
        assert(auto_array != NULL); \
 ...
 \
        the_verdict = name ## _make_space(auto_array, \
                                          auto_array->element_count + count); \
        if (the_verdict != MISSION_ACCOMPLISHED) \
            return the_verdict; \
 \
        if (count > 0) \
          { \
            memcpy(&(auto_array->array[auto_array->element_count]), \
                   new_elements, sizeof(type) * count); \
          } \
        auto_array->element_count += count; \
 \
        return MISSION_ACCOMPLISHED; \
 ...
 \
    extern verdict name ## _make_space(name *auto_array, \
 ...
 \
        if (auto_array->space >= (required_space + padding)) \
            return MISSION_ACCOMPLISHED; \
 \
        new_space = (((required_space + padding) * 2) + 3); \
        new_array = MALLOC_ARRAY(type, new_space); \
        if (new_array == NULL) \
            return MISSION_FAILED; \
 \
        element_count = auto_array->element_count; \
        if (element_count > 0) \
          { \
            memcpy(new_array, auto_array->array, \
                   sizeof(type) * element_count); \
          } \
        free(auto_array->array); \
        auto_array->array = new_array; \
        auto_array->space = new_space; \
 \
        return MISSION_ACCOMPLISHED; \
      } \
 \
    extern verdict name ## _insert(name *auto_array, size_t position, \
                                   type new_element) \
      { \
        verdict the_verdict; \
        size_t element_num; \
 \
        ...
        assert(auto_array->array != NULL); \
        assert(position >= 0); \
        assert(auto_array->element_count >= position); \
 \
        the_verdict = name ## _make_space(auto_array, \
                                          auto_array->element_count + 1); \
        if (the_verdict != MISSION_ACCOMPLISHED) \
            return the_verdict; \
 \
        element_num = auto_array->element_count; \
        while (element_num > position) \
          { \
            auto_array->array[element_num] = \
                    auto_array->array[element_num - 1]; \
            --element_num; \
          } \
        assert(element_num == position); \
        auto_array->array[position] = new_element; \
        ++(auto_array->element_count); \
 \
        return MISSION_ACCOMPLISHED; \
      } \
 \
    extern verdict name ## _insert_array(name *auto_array, size_t position, \
            size_t count, const name ## _element_type *new_elements) \
      { \
        verdict the_verdict; \
        size_t element_num; \
 \
 ...
 \
        ...
        if (the_verdict != MISSION_ACCOMPLISHED) \
            return the_verdict; \
 \
        element_num = ((auto_array->element_count + count) - 1); \
        while (element_num >= position + count) \
          { \
            auto_array->array[element_num] = \
                    auto_array->array[element_num - count]; \
            --element_num; \
          } \
        assert(element_num == ((position + count) - 1)); \
        memcpy(&(auto_array->array[position]), new_elements, \
               sizeof(type) * count); \
        auto_array->element_count += count; \
 \
        return MISSION_ACCOMPLISHED; \
 ...
 \
    extern void name ## _delete_last_one(name *auto_array) \
      { \
        assert(auto_array != NULL); \
        assert(auto_array->array != NULL); \
        assert(auto_array->element_count > 0); \
 \
        --(auto_array->element_count); \
      } \
 \
    extern void name ## _delete_one(name *auto_array, size_t position) \
      { \
        size_t element_num; \
 \
        assert(auto_array != NULL); \
 ...
 \
        ...
        while (element_num < auto_array->element_count) \
          { \
            auto_array->array[element_num - 1] = \
                    auto_array->array[element_num]; \
            ++element_num; \
          } \
        assert(element_num == auto_array->element_count); \
        --(auto_array->element_count); \
      } \
 \
    extern void name ## _delete_last_multiple(name *auto_array, size_t count) \
      { \
        assert(auto_array != NULL); \
        assert(auto_array->array != NULL); \
        assert(count >= 0); \
        assert(auto_array->element_count >= count); \
 \
        auto_array->element_count -= count; \
      } \
 \
    extern void name ## _delete_multiple(name *auto_array, size_t position, \
                                         size_t count) \
      { \
        size_t element_num; \
 \
        ...
        assert(auto_array->array != NULL); \
        assert(position >= 0); \
        assert(auto_array->element_count >= position + count); \
        assert(count >= 0); \
 \
        if (count == 0) \
            return; \
 \
        element_num = position; \
        while (element_num < auto_array->element_count - count) \
          { \
            auto_array->array[element_num] = \
                    auto_array->array[element_num + count]; \
            ++element_num; \
          } \
        assert(element_num == (auto_array->element_count - count)); \
        auto_array->element_count -= count; \
      } \
 \
    extern verdict name ## _replace(name *auto_array, size_t count, \
            const name ## _element_type *new_elements) \
      { \
        verdict the_verdict; \
 \
 ...
 \
        auto_array->element_count = 0; \
        the_verdict = name ## _make_space(auto_array, count); \
        if (the_verdict != MISSION_ACCOMPLISHED) \
            return the_verdict; \
 \
        if (count > 0) \
            memcpy(auto_array->array, new_elements, sizeof(type) * count); \
        auto_array->element_count = count; \
 \
        return MISSION_ACCOMPLISHED; \
      }
...
#endif /* AUTO_ARRAY_IMPLEMENTATION_H */
