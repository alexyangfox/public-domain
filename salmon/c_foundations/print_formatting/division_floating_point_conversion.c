/* file "division_floating_point_conversion.c" */

/*
 *  This file contains the implementation of one particular algorithm to do
 *  floating-point conversion to ASCII decimal.  This algorithm uses floating-
 *  point arithmetic, in particular division, to do the conversion itself, and
 *  provides the conversion through the interface used by the print formatting
 *  module.
 *
...
#define DECLARE_GENERATE_DIGITS_FUNCTION(function_name, type) \
static verdict function_name(floating_point_output_control *output_control, \
        type value, size_t *requested_mantissa_digit_count, \
        char **digit_buffer, boolean *exponent_is_negative, \
        char **exponent_digits, size_t *exponent_digit_count, \
        boolean *have_exponent_size_t, size_t *exponent_size_t, \
        boolean mantissa_is_negative, const char **special_value_string)


DECLARE_GENERATE_DIGITS_FUNCTION(generate_float_digits, float);
DECLARE_GENERATE_DIGITS_FUNCTION(generate_double_digits, double);
DECLARE_GENERATE_DIGITS_FUNCTION(generate_long_double_digits, long double);


#define DEFINE_GENERATE_DIGITS_FUNCTION(function_name, type, type_max, \
                                        type_min) \
 \
AUTO_ARRAY(function_name ## _auto, type); \
AUTO_ARRAY_IMPLEMENTATION(function_name ## _auto, type, 0); \
 \
static verdict function_name(floating_point_output_control *output_control, \
        type value, size_t *requested_mantissa_digit_count, \
        char **digit_buffer, boolean *exponent_is_negative, \
 ...
 \
    ...
    assert(exponent_size_t != NULL); \
    assert(special_value_string != NULL); \
 \
    if (value > type_max) \
      { \
        *special_value_string = "+inf"; \
    ...
    if (value == ((type)0.0) / ((type)0.0)) \
      { \
        *special_value_string = "+nan"; \
        return MISSION_ACCOMPLISHED; \
      } \
    if (value != value) \
      { \
 ...
 \
    ...
    if (value == (type)0) \
      { \
        code_point(501); \
        exponent_value = (double)0; \
        mantissa_remainder = (type)0; \
      } \
    else \
      { \
        code_point(502); \
        exponent_value = (double)0; \
        mantissa_remainder = value; \
        if (mantissa_remainder > ((type)DBL_MAX)) \
          { \
            static function_name ## _auto power_array; \
            static function_name ## _auto power_exponent_array; \
            static boolean power_array_initialized = FALSE; \
 \
            size_t index; \
 \
            code_point(503); \
            if (!power_array_initialized) \
              { \
                code_point(504); \
                function_name ## _auto_init(&power_array, 10); \
                function_name ## _auto_init(&power_exponent_array, 10); \
                power_array_initialized = TRUE; \
              } \
            else \
              { \
                code_point(505); \
 ...
 \
            ...
            while (TRUE) \
              { \
                type test_value; \
                double test_exponent; \
 \
                code_point(507); \
                if (power_array.element_count <= index) \
                  { \
                    code_point(508); \
                    assert(power_array.element_count == index); \
                    assert(power_exponent_array.element_count == index); \
 \
                    if (index == 0) \
                      { \
                        code_point(509); \
                        test_value = (type)(pow((double)10, \
                                                (double)DBL_MAX_10_EXP)); \
                        test_exponent = (double)DBL_MAX_10_EXP; \
                      } \
                    else \
                      { \
                        code_point(510); \
                        test_value = power_array.array[index - 1]; \
                        test_exponent = \
                                power_exponent_array.array[index - 1]; \
                        if ((type_max / test_value) < test_value) \
                            break; \
 ...
 \
                    code_point(511); \
                    function_name ## _auto_append(&power_array, test_value); \
                    function_name ## _auto_append(&power_exponent_array, \
                                                  test_exponent); \
 \
                    assert(power_array.element_count == (index + 1)); \
                    assert(power_exponent_array.element_count == \
                           (index + 1)); \
                  } \
                else \
                  { \
                    ...
                    assert(power_exponent_array.element_count > index); \
 \
                    test_value = power_array.array[index]; \
                    test_exponent = power_exponent_array.array[index]; \
                  } \
 \
                code_point(513); \
                if (test_value > mantissa_remainder) \
                  { \
                    code_point(514); \
                    break; \
                  } \
 ...
 \
                ...
                test_exponent = power_exponent_array.array[index]; \
 \
                while (mantissa_remainder > test_value) \
                  { \
                    code_point(518); \
                    exponent_value += test_exponent; \
                    mantissa_remainder /= test_value; \
                  } \
 \
                code_point(519); \
                if (index == 0) \
 ...
 \
        ...
        code_point(524); \
        if (mantissa_remainder < ((type)DBL_MIN)) \
          { \
            static function_name ## _auto power_array; \
            static function_name ## _auto power_exponent_array; \
            static boolean power_array_initialized = FALSE; \
 ...
 \
                    if (index == 0) \
                      { \
                        ...
                        test_exponent = (double)DBL_MIN_10_EXP; \
                      } \
                    else \
                      { \
                        code_point(532); \
                        ...
                        test_exponent = \
                                power_exponent_array.array[index - 1]; \
                        if ((type_min / test_value) > test_value) \
                            break; \
                        test_value = test_value * test_value; \
                        test_exponent *= 2; \
                      } \
 ...
 \
        ...
        code_point(546); \
        exponent_value += floor(log10((double)mantissa_remainder)); \
        if (exponent_value >= (double)DBL_MIN_10_EXP) \
          { \
            code_point(547); \
            mantissa_remainder /= (type)(pow((double)10, exponent_value)); \
          } \
        else \
          { \
            code_point(548); \
            assert((exponent_value + 1) >= (double)DBL_MIN_10_EXP); \
            mantissa_remainder /= \
                    (type)(pow((double)10, (exponent_value + 1))); \
            mantissa_remainder /= (type)10; \
          } \
        code_point(549); \
      } \
 \
    code_point(550); \
    if (exponent_value < (double)0) \
      { \
        code_point(576); \
 ...
 \
    code_point(578); \
    if (exponent_value > (double)~(size_t)0) \
      { \
        code_point(575); \
        *have_exponent_size_t = FALSE; \
 \
        *exponent_digits = &((*digit_buffer)[0]); \
        *exponent_digit_count = 0; \
        while ((*exponent_digit_count < 2) || (exponent_value > 0)) \
          { \
            double new_exponent; \
            double new_digit; \
 \
            ...
            new_exponent = floor(exponent_value / 10); \
            new_digit = (exponent_value - (new_exponent * (double)10)); \
            assert((new_digit >= 0) && (new_digit <= 9)); \
            **exponent_digits = ((int)new_digit) + '0'; \
            exponent_value = new_exponent; \
          } \
 \
        code_point(334); \
      ...
      { \
        code_point(334); \
        *exponent_size_t = (size_t)exponent_value; \
        *have_exponent_size_t = TRUE; \
 \
        the_verdict = early_exponent_by_size_t(output_control, \
                mantissa_is_negative, *exponent_is_negative, \
                *exponent_size_t, requested_mantissa_digit_count); \
      } \
 ...
 \
    code_point(279); \
 \
    for (char_num = 0; char_num < *requested_mantissa_digit_count; \
         ++char_num) \
      { \
        double this_digit; \
 \
        code_point(558); \
        this_digit = floor((double)mantissa_remainder); \
        assert((this_digit >= (double)0) && (this_digit < (double)10)); \
        assert((((int)this_digit) >= 0) && (((int)this_digit) < 10)); \
        the_verdict = handle_mantissa_digit(output_control, \
                                            (((int)this_digit) + '0')); \
        if (the_verdict != MISSION_ACCOMPLISHED) \
            return the_verdict; \
        code_point(377); \
        mantissa_remainder -= (type)this_digit; \
        mantissa_remainder *= (type)10; \
      } \
 \
    code_point(378); \
...
DEFINE_GENERATE_DIGITS_FUNCTION(generate_float_digits, float, FLT_MAX,
                                FLT_MIN);
DEFINE_GENERATE_DIGITS_FUNCTION(generate_double_digits, double, DBL_MAX,
                                DBL_MIN);
DEFINE_GENERATE_DIGITS_FUNCTION(generate_long_double_digits, long double,
                                LDBL_MAX, LDBL_MIN);


extern verdict do_division_floating_point_conversion(void *plug_in_data,
...
#define CLEAN_UP_ON_FAILURE() \
    ...
    /*
     * Next, we're going to generate the exponent digits into a buffer.  The
     * maximum number of exponent digits we might encounter is given by
     * MAX_EXPONENT_DIGITS.
     */
    code_point(257);
    buffer_size_bound = MAX_EXPONENT_DIGITS;
        ...
        case FPTK_FLOAT:
          {
            ...
            the_verdict = generate_float_digits(output_control, value,
                    &requested_mantissa_digit_count, &digit_buffer,
                    &exponent_is_negative, &exponent_digits,
                    &exponent_digit_count, &have_exponent_size_t,
                    &exponent_size_t, mantissa_is_negative,
        ...
        case FPTK_DOUBLE:
          {
            ...
            code_point(276);
            value = *(double *)value_data;

            the_verdict = generate_double_digits(output_control, value,
                    &requested_mantissa_digit_count, &digit_buffer,
                    &exponent_is_negative, &exponent_digits,
                    &exponent_digit_count, &have_exponent_size_t,
                    &exponent_size_t, mantissa_is_negative,
        ...
        case FPTK_LONG_DOUBLE:
          {
            ...
            code_point(292);
            value = *(long double *)value_data;

            the_verdict = generate_long_double_digits(output_control, value,
                    &requested_mantissa_digit_count, &digit_buffer,
                    &exponent_is_negative, &exponent_digits,
                    &exponent_digit_count, &have_exponent_size_t,
                    &exponent_size_t, mantissa_is_negative,
