/* file "sprintf_floating_point_conversion.c" */

/*
 *  This file contains the implementation of code to do conversion of floating-
 *  point values to decimal ASCII scientific notation using the sprintf()
 *  function.  It does this using the "floating_point_plug_in.h" interface for
 *  its input and the "floating_point_output_conversion.h" interface for its
 *  output, so it can be used by the print_formatting module.
 *
...
static verdict expand_floating_point_buffers(size_t expansion_amount,
        size_t *buffer_size_bound, char **allocated_buffer,
        char **digit_buffer);
static void sprintf_call_for_float(char *digit_buffer,
        size_t requested_mantissa_digit_count, void *value);
static void sprintf_call_for_double(char *digit_buffer,
        size_t requested_mantissa_digit_count, void *value);
static void sprintf_call_for_long_double(char *digit_buffer,
        size_t requested_mantissa_digit_count, void *value);
static verdict do_sprintf_digit_generation(
        floating_point_output_control *output_control,
        void (*type_specific_call)(char *digit_buffer,
                size_t requested_mantissa_digit_count, void *value),
        size_t *requested_mantissa_digit_count, char **digit_buffer,
        void *value, char **allocated_buffer, size_t *buffer_size_bound,
        boolean *have_exponent_size_t, const char **mantissa_digits,
        boolean *exponent_is_negative, char **exponent_digits,
        size_t *exponent_digit_count, boolean mantissa_is_negative,
        const char **special_value_string);
static void parse_and_fix_sprintf_output(char *to_parse,
        const char **mantissa_digits, size_t requested_mantissa_digit_count,
        boolean *exponent_is_negative, char **exponent_digits,
        size_t *exponent_digit_count, const char **special_value_string);
...
static verdict expand_floating_point_buffers(size_t expansion_amount,
  ...
  {
    code_point(582);
    if (*buffer_size_bound > ((~(size_t)0) - expansion_amount))
      {
        code_point(583);
        basic_error(
                "The exponent of the value for an `f' conversion specification"
                " plus the precision was so huge that not enough memory could "
                "be allocated to deal with it.");
        return MISSION_FAILED;
      }
    code_point(584);
    *buffer_size_bound += expansion_amount;

    if (*buffer_size_bound <= STACK_BUFFER_SIZE)
      {
        code_point(585);
    ...
    else
      {
        ...
        code_point(589);
        *allocated_buffer = MALLOC_ARRAY(char, *buffer_size_bound);
        if (*allocated_buffer == NULL)
          {
            code_point(590);
            return MISSION_FAILED;
...
static void sprintf_call_for_float(char *digit_buffer,
        size_t requested_mantissa_digit_count, void *value)
  {
    sprintf(digit_buffer, "%+#.*e", (int)(requested_mantissa_digit_count - 1),
            (double)*(float *)value);
  }
...
static void sprintf_call_for_long_double(char *digit_buffer,
        size_t requested_mantissa_digit_count, void *value)
  {
    sprintf(digit_buffer, "%+#.*Le", (int)(requested_mantissa_digit_count - 1),
            *(long double *)value);
  }
...
#define CLEAN_UP_ON_FAILURE() \
    ...
    verdict the_verdict;
    size_t buffer_size_bound;
    char *allocated_buffer;
    char *digit_buffer;
    void (*type_specific_call)(char *digit_buffer,
            size_t requested_mantissa_digit_count, void *value);
    boolean exponent_is_negative;
    boolean have_exponent_size_t;
    size_t exponent_size_t;
    char *exponent_digits;
    ...
    assert(requested_mantissa_digit_count > 0);
    assert(value_data != NULL);

    /*
     * Next, we're going to generate the mantissa digits and exponent digits
     * into a buffer.  The number of characters we need for the mantissa we've
     * already put in requested_mantissa_digit_count (in the `f' case, this may
     * have to be modified when we discover the exponent, but we'll worry about
     * that later if we have to).  The maximum number of exponent digits we
     * might encounter is given by MAX_EXPONENT_DIGITS.
     */
    if (requested_mantissa_digit_count > ((~(size_t)0) - MAX_EXPONENT_DIGITS))
      {
        code_point(256);
        too_many_floating_point_mantissa_digits(output_control);
        return MISSION_FAILED;
      }
    code_point(257);
    buffer_size_bound = (requested_mantissa_digit_count + MAX_EXPONENT_DIGITS);

    /*
     * If we're using sprintf() to generate the mantissa and exponent digits,
     * then there will be some other overhead characters generated into the
     * buffer:
     *
     *   * A sign character
     *   * A decimal point
     *   * An `e' character before the exponent
     *   * An exponent sign character
     *   * A closing zero character
     *
     * Also, when using sprintf() we need to pass in the number of mantissa
     * digits minus one as the precision, which must fit in an int.
     */

    code_point(258);
    if (((requested_mantissa_digit_count - 1) > (size_t)INT_MAX) ||
        (buffer_size_bound > ((~(size_t)0) - 5)))
      {
        code_point(259);
        too_many_floating_point_mantissa_digits(output_control);
        return MISSION_FAILED;
    ...
    switch (type_kind)
      {
        case FPTK_FLOAT:
          {
            type_specific_call = &sprintf_call_for_float;
            break;
          }
        case FPTK_DOUBLE:
          {
            type_specific_call = &sprintf_call_for_double;
            break;
          }
        case FPTK_LONG_DOUBLE:
          {
            type_specific_call = &sprintf_call_for_long_double;
            break;
          }
        default:
          {
    ...
    the_verdict = do_sprintf_digit_generation(output_control,
            type_specific_call, &requested_mantissa_digit_count, &digit_buffer,
            value_data, &allocated_buffer, &buffer_size_bound,
            &have_exponent_size_t, &mantissa_digits, &exponent_is_negative,
            &exponent_digits, &exponent_digit_count, mantissa_is_negative,
            &special_value_string);

    code_point(296);
    CLEAN_UP_ON_FAILURE();

    if (special_value_string != NULL)
      {
        verdict the_verdict;

        the_verdict = special_non_numeric_value(output_control,
                                                special_value_string);
    ...
    if (care_about_trailing_zero_count)
      {
        size_t count_without_zeros;

        code_point(336);
        count_without_zeros = requested_mantissa_digit_count;
        while ((count_without_zeros > 0) &&
               (mantissa_digits[count_without_zeros - 1] == '0'))
          {
            code_point(337);
            --count_without_zeros;
          }
        ...
        the_verdict = notify_of_mantissa_trailing_zero_count(output_control,
                (requested_mantissa_digit_count - count_without_zeros));
        requested_mantissa_digit_count = count_without_zeros;
        CLEAN_UP_ON_FAILURE();
      }
    ...
    for (digit_num = 0; digit_num < requested_mantissa_digit_count;
      ...
      {
        code_point(371);
        the_verdict = handle_mantissa_digit(output_control,
                                            mantissa_digits[digit_num]);
        if (the_verdict != MISSION_ACCOMPLISHED)
            break;
        code_point(377);
      }
    ...
    if (have_exponent_size_t)
      {
        the_verdict = late_exponent_by_size_t(output_control,
                exponent_is_negative, exponent_size_t);
      }
    else
      {
        the_verdict = late_exponent_by_digits(output_control,
                exponent_is_negative, exponent_digit_count, exponent_digits);
      }
    CLEAN_UP_ON_FAILURE();
...
static verdict do_sprintf_digit_generation(
        ...
        const char **special_value_string)
  {
    size_t old_requested_mantissa_digit_count;
    verdict the_verdict;

    code_point(267);

    assert(output_control != NULL);
    assert(type_specific_call != NULL);
    assert(requested_mantissa_digit_count != NULL);
    assert(digit_buffer != NULL);
    assert(*digit_buffer != NULL);
    assert(value != NULL);
    assert(allocated_buffer != NULL);
    assert(buffer_size_bound != NULL);
    assert(have_exponent_size_t != NULL);
    assert(mantissa_digits != NULL);
    assert(exponent_is_negative != NULL);
    assert(exponent_digits != NULL);
    assert(exponent_digit_count != NULL);
    assert(special_value_string != NULL);
    assert((*requested_mantissa_digit_count) > 0);

    (*type_specific_call)(*digit_buffer, *requested_mantissa_digit_count,
                          value);
    parse_and_fix_sprintf_output(*digit_buffer, mantissa_digits,
            *requested_mantissa_digit_count, exponent_is_negative,
            exponent_digits, exponent_digit_count, special_value_string);
    if (*special_value_string != NULL)
        return MISSION_ACCOMPLISHED;

    old_requested_mantissa_digit_count = *requested_mantissa_digit_count;

    the_verdict = early_exponent_by_digits(output_control,
            mantissa_is_negative, *exponent_is_negative, *exponent_digit_count,
            *exponent_digits, requested_mantissa_digit_count);
    if (the_verdict != MISSION_ACCOMPLISHED)
      {
        code_point(269);
        return the_verdict;
    ...
    code_point(279);
    if (*requested_mantissa_digit_count > old_requested_mantissa_digit_count)
      {
        code_point(494);
        the_verdict = expand_floating_point_buffers(
                (*requested_mantissa_digit_count -
                 old_requested_mantissa_digit_count), buffer_size_bound,
                allocated_buffer, digit_buffer);
        if (the_verdict != MISSION_ACCOMPLISHED)
          {
            code_point(493);
            return the_verdict;
...
static void parse_and_fix_sprintf_output(char *to_parse,
        ...
        size_t *exponent_digit_count, const char **special_value_string)
  {
    ...
    *mantissa_digits = follow;
    ...
    code_point(299);
    assert((follow - *mantissa_digits) == requested_mantissa_digit_count);
    assert(*follow == 'e');
    ++follow;
    ...
    *exponent_digits = follow;
    while ((*follow >= '0') && (*follow <= '9'))
        ++follow;
    *exponent_digit_count = (follow - *exponent_digits);
    assert(*exponent_digit_count >= 2);

    assert(*follow == 0);

    *special_value_string = NULL;
  }
