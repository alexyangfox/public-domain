/* file "bytewise_hex_pointer_conversion.h" */

/*
 *  This file contains the interface to one particular algorithm to do pointer
 *  conversion to ASCII.  This algorithm converts the pointer by going through
 *  its raw bytes and writing them out in hexadecimal, and provides the
 *  conversion through the interface used by the print formatting module.
 *
...
#ifndef BYTEWISE_HEX_POINTER_CONVERSION_H
#define BYTEWISE_HEX_POINTER_CONVERSION_H
 ...
 *      Usage
 *
 *  This interface provides exactly four function declarations.  Between them,
 *  they provide a pointer conversion algorithm that converts a pointer value
 *  to ASCII characters by converting the raw bytes of the pointer to
 *  hexadecimal.  The interface to this module is designed to allow it to be
 *  used as a plug-in with the interface provided by the "pointer_plug_in.h"
 *  header file.
 *
 *  The do_bytewise_hex_pointer_conversion() function is the one that does the
 *  actual conversion.  It has the type pointer_plug_in_function_type and its
 *  interface is as specified in the "pointer_plug_in.h" header file.
 *
 *  The create_bytewise_hex_pointer_conversion_data() function creates the data
 *  object to be passed as the first parameter to the
 *  do_bytewise_hex_pointer_conversion().  The data object must be created
 *  before the conversion function may be called.  Any number of data objects
 *  may be created and used in any order.  If the data object creation function
 *  returns NULL, it has failed and the conversion function may not be used.
 *
 *  The destroy_bytewise_hex_pointer_conversion_data() may be used to
 *  deallocate a data object when it is no longer needed.  It should only be
 *  called on the restult of successfully calling
 *  create_bytewise_hex_pointer_conversion_data(), and after it is called for
 *  an object, it may not be called again for that object and the conversion
 *  function may not be called again with that data object.
 *
 *  The bytewise_hex_pointer_conversion_max_output_characters() function
 *  returns the maximum number of characters in the output from the
 *  do_bytewise_hex_pointer_conversion() function.
 *
 *  So, to set this module as the plug-in to use,
 *  set_pointer_conversion_plug_in() should be called with
 *  do_bytewise_hex_pointer_conversion() as the first parameter; the result of
 *  calling create_bytewise_hex_pointer_conversion_data() as the second
 *  parameter; and the result of calling
 *  bytewise_hex_pointer_conversion_max_output_characters() as the third
 *  parameter.
 *
 *
 *      Error Handling
 ...
 *  do_bytewise_hex_pointer_conversion() function will handle any error
 *  condition by returning MISSION_FAILED.  The
 *  create_bytewise_hex_pointer_conversion_data() function will return NULL in
 *  case of a failure and non-NULL in case of success.  The
 *  destroy_bytewise_hex_pointer_conversion_data() and
 *  bytewise_hex_pointer_conversion_max_output_characters() functions cannot
...
extern verdict do_bytewise_hex_pointer_conversion(void *plug_in_data,
        char *output_buffer, void *value_data, size_t *output_byte_count);
extern void *create_bytewise_hex_pointer_conversion_data(void);
extern void destroy_bytewise_hex_pointer_conversion_data(void *plug_in_data);
extern size_t bytewise_hex_pointer_conversion_max_output_characters(void);


#endif /* BYTEWISE_HEX_POINTER_CONVERSION_H */
