/* file "bytewise_hex_pointer_conversion.c" */

/*
 *  This file contains the implementation of one particular algorithm to do
 *  pointer conversion to ASCII.  This algorithm converts the pointer by going
 *  through its raw bytes and writing them out in hexadecimal, and provides the
...
typedef struct bytewise_hex_pointer_conversion_static_data
  {
    boolean little_endian;
  } bytewise_hex_pointer_conversion_static_data;
...
extern verdict do_bytewise_hex_pointer_conversion(void *plug_in_data,
  ...
  {
    bytewise_hex_pointer_conversion_static_data *typed_static_data;
    boolean little_endian;
    union
      {
        void *pointer;
    ...
    code_point(465);
    if (plug_in_data == NULL)
        return MISSION_FAILED;

    typed_static_data =
            (bytewise_hex_pointer_conversion_static_data *)plug_in_data;
    assert(typed_static_data != NULL);

    little_endian = typed_static_data->little_endian;

    overlap.pointer = value_data;
    ...
    while (TRUE)
      {
        ...
        code_point(479);
        byte = ((overlap.array[byte_num] >> 4) & 0xf);
        output_buffer[(byte_num * 2) + 0] =
                ((byte <= 9) ? (byte + '0') : ((byte - 10) + 'a'));
        byte = ((overlap.array[byte_num] >> 0) & 0xf);
        output_buffer[(byte_num * 2) + 1] =
                ((byte <= 9) ? (byte + '0') : ((byte - 10) + 'a'));
    ...
    code_point(483);
    *output_byte_count = (byte_count * 2);
...
extern void *create_bytewise_hex_pointer_conversion_data(void)
  {
    bytewise_hex_pointer_conversion_static_data *result;
    union
      {
    ...
    result = MALLOC_ONE_OBJECT(bytewise_hex_pointer_conversion_static_data);
    if (result == NULL)
        return NULL;

    overlap.pointer = &(overlap.array[0]);
    test_char = overlap.array[0];
    ++(overlap.pointer);
    if (test_char == overlap.array[0])
      {
        code_point(466);
