/*****************************************************************************
DOS VGA/VBE graphics demo for DJGPP
...
#include <go32.h> /* __tb, _go32_info_block */
#include <dpmi.h> /* __dpmi_regs, __dpmi_int() */
#include <crt0.h> /* _CRT0_FLAG_NEARPTR, _crt0_startup_flags */
#include <dos.h> /* outportw(), outportb(), inportb() */

#if !defined(__DJGPP__)
#error Compile this file with DJGPP
#endif
...
/* set read plane
	...
	outportw(VGA_GC_INDEX, (p << 8) | 0x04);
/* set write plane
	outportb(VGA_SEQ_INDEX, 2);
	outportb(VGA_SEQ_DATA, pmask); */
	outportw(VGA_SEQ_INDEX, (pmask << 8) | 0x02);
}
/*****************************************************************************
...
/*****************************************************************************
*****************************************************************************/
#define	NEARPTR(S,O)	(char *)((S) * 16uL + (O) + __djgpp_conventional_base)
