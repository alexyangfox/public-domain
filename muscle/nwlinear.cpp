...
float NWLinear(Mx<float> &MatchMxf, string &Path)
	{
	Path.clear();

	const unsigned LA = MatchMxf.m_RowCount-1;
	const unsigned LB = MatchMxf.m_ColCount-1;
	float **MatchMx = MatchMxf.GetData();

	g_M.Alloc("NWLinearM", LA+1, LB+1);
	float **M = g_M.GetData();
	...
	for (unsigned i = 1; i <= LA; ++i)
		...
		float *ptrMiRow = M[i];
		float *ptrMi_1 = M[i-1];
		const float *MatchRow = MatchMx[i];
		for (unsigned j = 0; j < LB; ++j)
			{
			float MM = *ptrMi_1++ + MatchRow[j+1];
			float DM = *ptrMi_1;
			float IM = *ptrMiRow++;
			if (MM >= DM && MM >= IM)
				*ptrMiRow = MM;
	...
	for (;;)
		...
		if (i == 0 || j == 0)
			break;
		float MM = M[i-1][j-1] + MatchMx[i-1][j-1];
		float DM = M[i-1][j];
		float IM = M[i][j-1];
		float XM = M[i][j];
		if (XM == MM)
